...
func (cmd *Commands) Purge(s *discordgo.Session, m *discordgo.Message, ctx *Context) {
	...
	if err != nil || amount > 100 {
		s.ChannelMessageSend(m.ChannelID, "You have to specify a number and it has to be inbetween 1-100")
		return
	}
	...
	if err != nil {
		s.ChannelMessageSend(m.ChannelID, "Could not fetch channel messages")
		return
	}
...
func (cmd *Commands) Unban(s *discordgo.Session, m *discordgo.Message, ctx *Context) {
	...
	s.ChannelMessageSendEmbed(m.ChannelID, &discordgo.MessageEmbed{
		Description: fmt.Sprintf("Unbanned %d members | Didn't unban %d account(s) banned by %s.", unbancount, nukebot, s.State.User.Username),
